...
AREAS: dict[str, BBox] = {
    # whole greater-Athens basin: Piraeus + south coast → northern suburbs →
    # the foot of Hymettus (≈534 km²). Use "athens-center" for a fast,
    # street-level study of just the historic core.
    "athens": BBox(min_lon=23.600, min_lat=37.840, max_lon=23.820, max_lat=38.090),
...
def area_km2(bb: BBox) -> float:
    mid = math.radians((bb.min_lat + bb.max_lat) / 2)
    w = (bb.max_lon - bb.min_lon) * 111.0 * math.cos(mid)
    h = (bb.max_lat - bb.min_lat) * 111.0
    return w * h


def suggest_resolution(bb: BBox) -> int:
    """Street-level (res 9, ~175 m) for a neighbourhood; district-level (res 8,
    ~460 m) for anything metro-scale, to keep the hex count and CPU sane."""
    return 8 if area_km2(bb) > 80 else 9

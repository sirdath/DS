...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // ─── Hero wordmark — draw-in; mode (filled/gradient/outline/laser) set by the style switcher ───
      ...
      if (heroLogo) {
        ...
        const runLogo = (mode: string) => {
          ...
          // ── Laser: burn the outline on hot, then let it settle into a calm purple outline ──
          if (mode === "laser") {
            strokes.forEach((p) => {
          ...
          // ── Filled / gradient / outline: plain stroke draw-in (fill cross-fades in via .drawn) ──
          if (reduceMotion || strokes.length === 0) {
      ...
      <section className="hero">
        <div className="hero-logo-wrap" id="hero-logo-wrap">
          ...
          >
            ...
            <circle className="laser-head" id="laser-head" r="8" cx="0" cy="0" />
          </svg>
        </div>

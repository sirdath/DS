...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // ─── Hero wordmark — purple laser beams in from the corner, traces the outline ───
      ...
      if (heroLogo) {
        ...
        if (reduceMotion || strokes.length === 0) {
          // static glowing outline, no laser
          ...
          strokes.forEach((p) => {
            ...
            tl.to(st, {
              ...
              ease: "power2.inOut",
              onStart: () => {
                if (head) { head.setAttribute("cx", String(start.x)); head.setAttribute("cy", String(start.y)); gsap.to(head, { autoAlpha: 1, duration: 0.2 }); }
                if (beam) { beam.setAttribute("x2", String(start.x)); beam.setAttribute("y2", String(start.y)); gsap.to(beam, { autoAlpha: 1, duration: 0.2 }); }
              },
              onUpdate: () => {
          ...
          if (beam && head) tl.to([beam, head], { autoAlpha: 0, duration: 0.5 }, ">0.15");
          disposers.push(() => tl.kill());
        }
      }
      ...
      <section className="hero">
        <div className="hero-logo-wrap" id="hero-logo-wrap">
          ...
          >
            ...
            <line className="laser-beam" id="laser-beam" x1="1500" y1="-350" x2="1500" y2="-350" />
            <circle className="laser-head" id="laser-head" r="10" cx="0" cy="0" />
          </svg>
        </div>
